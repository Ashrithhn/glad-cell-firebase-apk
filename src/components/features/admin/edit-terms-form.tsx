...
export function EditTermsForm({ currentContent }: EditTermsFormProps) {
  ...
  async function onSubmit(values: FormData) {
    ...
    try {
      const result = await updateContent('terms-and-conditions', values.content);

      if (result.success) {
        toast({
          title: 'Terms & Conditions Updated!',
          description: 'The terms and conditions content has been saved.',
        });
        router.refresh(); 
      } else {
        throw new Error(result.message || 'Failed to update terms and conditions.');
      }
    } catch (error) {
      toast({
        title: 'Update Failed',
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea placeholder="Enter the content for the Terms &amp; Conditions page..." {...field} rows={20} />
                </FormControl>
                 <FormDescription>
