...
interface EditHelpFormProps {
  currentContent: string;
}

export function EditHelpForm({ currentContent }: EditHelpFormProps) {
  ...
  async function onSubmit(values: FormData) {
    ...
    try {
      const result = await updateContent('help', values.content); 

      if (result.success) {
        toast({
          title: 'Help Content Updated!',
          description: 'The "Help/FAQ" page content has been saved.',
          variant: 'default',
        });
        router.refresh(); 
      } else {
        throw new Error(result.message || 'Failed to update help content.');
      }
    } catch (error) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <fieldset disabled={isDisabled} className="space-y-6">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Help/FAQ Content</FormLabel>
                <FormControl>
                  <Textarea placeholder="Enter the content for the Help/FAQ page..." {...field} rows={20} />
                </FormControl>
                 <FormDescription>
                   Use Markdown for formatting (e.g., # Heading, **bold**, *italic*, lists, links).
                 </FormDescription>
                <FormMessage />
              </FormItem>
            )}
